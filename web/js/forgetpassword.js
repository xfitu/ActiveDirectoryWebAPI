...
/* This jquery file is used to send ajax data to forgetpassword.jsp on the server*/
...
if(fname!==""&&lname!==""&&username!==""&&email!==""){ 
   ...
   // alert("Name:"+name +" "+"email:"+email+" "+"username:"+ username);
    $.ajax({
                type: "POST",
                url: "https://localhost:7443/ActiveDirectoryWebAPI/ForgetPassword",
                data: dataString,
                dataType: "json",
...
function SendEmail(fname,lname,email){
  ...
  var verificationlink = "https://localhost:7443/ActiveDirectoryWebAPI/resetpassword.jsp";  
  dataString ="subject="+subject+"&body="+body+"&link="+verificationlink+ "&fname=" + fname+"&lname="+lname+"&receiveremail="+email;
     $.ajax({
            type: "POST",
            url: "https://localhost:7443/ActiveDirectoryWebAPI/SendEmail",
            data: dataString,
            dataType: "json", 
            success: function( result) {
...
function ShowSuccessMessage(result){
 if(result.Status==="success"){
    ...
    $('#modalbodycontent').html("<p><strong>send request "+result.Status+"</strong></p>"+
       ...
       +"<p> go to<a href=login.jsp> login "+"</a></p>");
